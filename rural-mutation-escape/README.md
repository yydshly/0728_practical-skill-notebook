...
当前 v0.1 已经完成从可玩原型、真实演示、技术说明、自动验证到远端发布的阶段性闭环。以下内容是后续可以继续研究的路线，并非当前已交付能力；对应实现尚未启动，也没有承诺发布日期。

### A：补齐真正的游戏闭环

这是建议优先恢复的方向：

- 为敌人增加视野、听觉、遮挡判断和短期记忆；
- 增加攻击前摇、捕获、失败状态和明确反馈；
- 建立确定性检查点与可靠重试；
- 把南门升级为可交互、带动态碰撞和正式结算的出口；
- 在章节完成后停机，或提供明确的重新游玩入口。

目标不是立刻扩大地图，而是把现有村庄做成一段可失败、可恢复、可重复游玩的 2–4 分钟垂直切片。

### B：提升美术与体感

核心闭环稳定后，再逐步替换或增强程序化占位内容：

- 提升人物、居民和变异体模型、骨骼、动作与表情；
- 补充农村建筑、道路、植被、材质和环境细节；
- 加强灯光、雾效、粒子、危险反馈和镜头构图；
- 替换资产时继续遵守碰撞、任务锚点、来源记录和性能预算。

这一路线不建议先于游戏闭环大规模展开，否则尚未稳定的玩法流程容易造成美术返工。

### C：沉淀大模型驱动的生成流程

地图、任务和角色接口稳定后，可以研究一条受约束、可验证的生成管线：

- 从故事描述生成地图数据、区域、道路和地标；
- 生成 NPC、敌人、任务链、对白与触发条件；
- 用 schema、约束和确定性校验限制模型输出；
- 同步生成单元测试、浏览器证据场景和验收清单；
- 记录提示词、参考来源、生成产物与人工审查结果。

目标是形成“故事输入 → 结构化设计 → 可运行内容 → 自动验证 → 人工审查”的开发流程，而不是允许模型无约束地修改整个游戏。
...
先稳定可玩的规则和内容接口，再提升视觉品质，最后把稳定接口抽象为可复用的生成流程。本轮只记录这三条路线，全部保持暂缓实施状态。
