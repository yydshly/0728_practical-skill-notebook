...
第二条命令是只读存在性审计，会为每个选定技能输出 `name`、`installed` 和 `installPath`；任何一项缺失都会返回非零状态。候选提交 `43ef034d06f341f525740fbfd243a6d9aabc0b20` 的安装器自动化回归为 `4/4`：覆盖锁定引用与文本换行规范化、已安装副本漂移、脏子模块拒绝和二进制原始字节漂移。回归测试只使用临时 `USERPROFILE` 和临时 Git 仓库，不会写入真实的 `C:\Users\yun68\.codex\skills`。
