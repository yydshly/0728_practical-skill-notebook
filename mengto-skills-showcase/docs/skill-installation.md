...
本项目只选用 `config/selected-skills.json` 批准的 16 个 Skill。Skill 是 Codex 开发与验收时读取的工作说明，也就是开发操作规范：帮助它在规划、实现、测试和交付时遵循合适的工作流。网页运行时不会加载这些 Skill；它们**不是运行时依赖**，不会被打包进游戏，也不会成为浏览器端代码的依赖。最终交付仍是普通 Vite/Three.js 网页产品。

本项目代码位于当前 `mengto-skills-showcase` 套件目录，不会写进 `C:\Users\yun68\.codex\skills` 中的 Skill 文件。雾屿灯塔与 Typographic Flags 是套件外的独立演示，不属于能力展厅三产品，也不会因为安装这些 Skill 而被复制或改写。
...
- 本地子模块 HEAD 等于锁定提交；
- 本地子模块工作树完全干净，包括没有已修改文件和未跟踪文件；
- 父仓库记录的 Git 子模块指针也等于同一锁定提交；
- 每个批准来源目录都包含 `SKILL.md`。

对已安装副本，脚本会按“相对路径 + 每个文件的 SHA-256”计算完整目录指纹，并与锁定来源逐项比较。已知文本扩展名且内容为有效 UTF-8 时，比较前会把 `CRLF` 和单独的 `CR` 统一为 `LF`，避免 Windows 检出与官方安装器使用不同换行符时产生误报；二进制文件和无法按 UTF-8 解码的文件仍按原始字节精确比较：
...
第二条命令是只读存在性审计，会为每个选定技能输出 `name`、`installed` 和 `installPath`；任何一项缺失都会返回非零状态。历史候选 `43ef034d06f341f525740fbfd243a6d9aabc0b20` 的安装器自动化回归为 `4/4`：覆盖锁定引用与文本换行规范化、已安装副本漂移、脏子模块拒绝和二进制原始字节漂移。这是历史证据，不冒充后续候选的重跑。回归测试只使用临时 `USERPROFILE` 和临时 Git 仓库，不会写入真实的 `C:\Users\yun68\.codex\skills`。

Task 13 工作候选（基线 `10d99e1307bce62c279c05c5b6650f65a5f403ab` 加文档/契约差异）新鲜执行 `node scripts/check-selected-skills.mjs`，批准清单的 16/16 项均报告 `installed=true`。这是只读存在性结果，不等于重新执行安装器，也不会把历史 `4/4` 改写成本轮安装结果；最终 Task 13 提交 SHA 由 Git 历史和报告登记。
