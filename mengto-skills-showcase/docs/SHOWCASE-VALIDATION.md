...
- 内容候选：`1a610db03487b8c2124f2815bb20b03af492b721`。这是写入本节前、包含最终 GIF、全部已审查 corrective 提交和 final-review 修复的完整 implementation SHA。
- 最终 GIF：`docs/demos/07-mengto-skills-showcase.gif`，实际 `701801` bytes，blob `0eeb8357a03d5a43921ff5c928658141211bccf1`；强化后的无依赖 parser 逐块验证签名 `GIF89a`、逻辑屏 `720×480`、`60` 个 image frame、每帧 `25` centiseconds、总时长 `15` 秒、`4` FPS、`NETSCAPE2.0` loop count `0`、颜色表/边界、完整 LZW 像素流、trailer 和无尾随字节。LZW EOI 后只接受 `1–8` 个全零 padding bits，其中真实 FFmpeg 的第 14、37 帧各有一个字节对齐的 `0x00` flush byte；非零或超过 8 bits 均由回归测试拒绝。最终重录提交仍为 `34b17c1fa59adbb1dbb9cebcab2c10fe32a3a03e`，本轮 GIF 无 diff。
...
- 最终默认并行单元门禁：第一次 `npm test` 为 `49/49` files、`728/728` tests，Vitest `36.07s`（墙钟 `37.049s`）；第二次为 `49/49` files、`728/728` tests，Vitest `37.30s`（墙钟 `38.142s`）。两次均在 final-review implementation SHA 上新鲜完整运行、首次通过且没有 retry。
- 构建与组合往返：普通 `npm run build` exit 0、墙钟 `13.945s`（保留一个非阻塞的 `>500 kB` chunk warning）；`npm run build:showcase` exit 0、墙钟 `22.509s`；`npm run test:showcase-preview` 在本地 `127.0.0.1:4292` 单一组合源站为 `4/4` passed（Playwright `46.2s`、墙钟 `48.186s`），覆盖三个产品的同标签返回及唯一组合预览源站。
- 根级归档门禁：两个根测试文件合计 `44/44` passed（Node test `2.986s`、墙钟 `3.110s`）；第 07 项 checker 输出 `MengTo project 07 check passed: GIF89a, 720px, 701801 bytes.`；`git diff --check` exit 0；`4292`、`5277`、任务相关进程和 `mengto-showcase-recording-*` 临时目录均为空。
- corrective 记录：初始完整并行门禁先后暴露 self-closing semantic readiness 的 5 秒外层超时、held-open body 探针未在 5 秒内命中，以及 SIGTERM 清理先后超过 5 秒和 15 秒外层预算；这些失败均未计入上面的两次最终 PASS。corrective A `0bd92e4fef8f746299b50eff1026e43241b0d4d0` 只调整对应参数化测试的外层 harness；corrective C `78c4031c70758cc3d0ff3fa5f178a914d9ffba25` 为 readiness fetch/body 增加全局信号和每次 500ms 边界；corrective D `72e5e024714b6986ed574f06e289bb363785627d` 的纯 test-harness 诊断随后被 15 秒真实失败否定；corrective E `adee052c078f8f327f58a50843558d635c2c0b54` 改为对 live Windows 根进程树执行一次递归终止，并只在根进程已消失时使用 CIM orphan fallback，同时增加两条确定性策略测试。final-review corrective `1a610db03487b8c2124f2815bb20b03af492b721` 用严格 GIF block/LZW parser 取代 10-byte 头部检查，并以统一 settlement helper 按 browser → preview → temp 顺序独立尝试清理：单一错误保留原对象，多错误以 primary、browser、preview、temp 的确定顺序聚合，且 primary 不再被清理错误覆盖。A、C、D、E 与 final-review corrective 均已独立验证，E 仍取代 D 作为进程树根因修正。
